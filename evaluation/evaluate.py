...
# ---------------------------------------------------------------------------
GROUND_TRUTH_PATH = Path(_root) / "config" / "contract_ground_truth.json"
...
def _load_ground_truth() -> Dict[str, Dict]:
    ...
    if not GROUND_TRUTH_PATH.exists():
        ...
        print("    Run 'python evaluation/generate_test_pdfs.py' first to generate it.")
        sys.exit(1)
...
def _contract_pdf(contract_id: str) -> str:
    return os.path.join(_root, "data", f"Contract_{contract_id}.pdf")
...
class EvaluationRunner:
    ...
    def __init__(
        ...
        questions: List[str],
        conditions: List[str],
        output_dir: str = os.path.join(_root, "evaluation", "results", "baseline"),
        resume: bool = False,
        dry_run: bool = False,
    ):
...
Examples:
  ...
  python evaluate.py --resume                    # Skip completed queries
    ...
    parser.add_argument(
        "--output-dir", default=os.path.join(_root, "evaluation", "results", "baseline"),
        help="Output directory (default: evaluation/results/baseline).",
    )
    parser.add_argument(
